  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="px-4 py-6">
        ...
        {featuredRecipe && (
          <Link
            href={`/mobile/recipes/${encodeURIComponent(
              featuredRecipe.name.toLowerCase().replace(/\s+/g, "-")
            )}`}
          >
            ...
            <h3 className="font-serif font-bold text-xl">{featuredRecipe.name}</h3>
            <div className="flex items-center text-sm text-gray-500 mt-1">
              <span>
                {featuredRecipe.prep_time_minutes +
                  featuredRecipe.cook_time_minutes} minutes
              </span>
            </div>
          </Link>
        )}
            ...
            >
              <div className="relative w-24 h-24 rounded-lg overflow-hidden flex-shrink-0">
                <Image
                  src={
                    recipe.image_url ||
                    `/placeholder.svg?height=100&width=100&query=${recipe.name}`
                  }
                  alt={recipe.name}
                  fill
